...
export default function Dashboard({ initial }: { initial: DashboardPayload }) {
  ...
  const selectedValidated = selected?.validation_status === "validated";
    ...
    <main className="main">
      ...
      <section className="panel" aria-labelledby="decision-path-title">
        ...
        <div className="contract-grid" aria-label="GRIDPULSE evidence stages">
          ...
          <div><span>03 / PREDICTED</span><strong>AZURE MACHINE LEARNING</strong><b className={intelligence ? "ok" : aiReady ? "warn" : "warn"}>{intelligence ? "MODEL OUTPUT" : aiReady ? "READY TO RUN" : "CONFIG REQUIRED"}</b></div>
