...
const EMPTY: DashboardPayload = {
  ...
  summary: { reports: 0, validated: 0, pending: 0, regions: 0, last_report: null, active_regions: 0 },
  events: [],
  regions: [],
};
...
function project(lat: number, lon: number) {
  return {
    ...
    y: Math.max(5, Math.min(95, ((90 - lat) / 180) * 100)),
  };
}
  ...
  return (
    <div className="shell">
      <header className="top">
        ...
        <div className={offline ? "system-state warn" : "system-state"}>
          <span className="pulse-dot" aria-hidden="true" /> {offline ? "DEGRADED CONNECTION" : "SYSTEM OPERATIONAL"}
        </div>
      </header>
      ...
      <main className="main">
        <section className="hero-row">
          <div>
            <div className="eyebrow">INFRASTRUCTURE OBSERVABILITY / LIVE CONTROL PLANE</div>
            <h1>See disruption before it becomes a blind spot.</h1>
            <p className="hero-copy">GRIDPULSE correlates distributed telemetry, validates independent observations, and prepares the evidence layer for AI-powered grid intelligence.</p>
          </div>
        ...
        <section className="control-grid">
          <article className="panel map-panel">
            ...
            <div className="map" role="img" aria-label="Global telemetry map. Select a telemetry point to inspect its validation state.">
              ...
              <div className="equator" aria-hidden="true" />
              {data.events.map((event) => {
                const point = project(event.latitude, event.longitude);
                return <button key={event.id} className={`node node-${event.status} ${selected?.id === event.id ? "node-selected" : ""}`} style={{ left: `${point.x}%`, top: `${point.y}%` }} onClick={() => setSelected(event)} aria-label={`${statusLabel(event.status)} report in ${event.region_name}, ${Math.round(Number(event.confidence) * 100)} percent confidence`} />;
              })}
          ...
          <aside className="panel incident-panel" aria-live="polite">
            ...
            {selected ? <>
              ...
              <div className="coordinate">{selected.latitude.toFixed(4)}°, {selected.longitude.toFixed(4)}°</div>
              <dl className="evidence-list">
                <div><dt>VALIDATION</dt><dd>{statusLabel(selected.validation_status)}</dd></div>
                <div><dt>SOURCE</dt><dd>{statusLabel(selected.source)}</dd></div>
                <div><dt>OBSERVED</dt><dd>{formatTime(selected.observed_at)}</dd></div>
              </dl>
        ...
        <section className="lower-grid">
          <article className="panel">
            <div className="panel-heading"><div><div className="label">EVENT STREAM</div><div className="panel-title">LATEST OBSERVATIONS</div></div><button className="ghost-button" onClick={refresh} disabled={loading}>{loading ? "SYNCING…" : "SYNC NOW"}</button></div>
          ...
          <article className="panel">
            ...
            <div className="panel-title coverage-title">OBSERVATION CLUSTERS</div>
        ...
        <section className="intelligence-strip">
          <div><span className="ai-mark">AI</span><div><div className="label">INTELLIGENCE LAYER</div><strong>READY FOR MODEL + EXPLANATION SERVICES</strong></div></div>
          <p>Structured evidence is already separated from the future AI layer: predictions will carry a model version, timestamp, horizon, confidence, and grounded explanation.</p>
        </section>

        <section className="network-status panel">
          <div className="label">SYSTEM CONTRACT</div>
          <div className="contract-grid">
            <div><span>INGESTION</span><strong>POST /api/telemetry/report</strong><b className="ok">READY</b></div>
            <div><span>VALIDATION</span><strong>POSTGRESQL CORROBORATION</strong><b className="ok">READY</b></div>
            <div><span>INTELLIGENCE</span><strong>AZURE AI / GROUNDED OUTPUT</strong><b className="warn">INTEGRATION NEXT</b></div>
            <div><span>LAST OBSERVED</span><strong>{formatTime(data.summary.last_report)}</strong><b className={data.summary.last_report ? "ok" : "warn"}>{data.summary.last_report ? "INGESTED" : "NO DATA"}</b></div>
          </div>
        </section>
      </main>
