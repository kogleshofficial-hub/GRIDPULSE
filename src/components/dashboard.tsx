...
type AIHealth = { ok: boolean; services: { azureMachineLearning: boolean; microsoftFoundry: boolean } };
...
export default function Dashboard({ initial }: { initial: DashboardPayload }) {
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <section className="network-status panel"><div className="label">SYSTEM CONTRACT</div><div className="contract-grid"><div><span>INGESTION</span><strong>POST /api/telemetry/report</strong><b className="ok">READY</b></div><div><span>VALIDATION</span><strong>POSTGRESQL CORROBORATION</strong><b className="ok">READY</b></div><div><span>INTELLIGENCE</span><strong>AZURE ML + FOUNDRY</strong><b className={aiReady ? "ok" : "warn"}>{aiReady ? "READY" : "CONFIG REQUIRED"}</b></div><div><span>LAST OBSERVED</span><strong>{formatTime(data.summary.last_report)}</strong><b className={data.summary.last_report ? "ok" : "warn"}>{data.summary.last_report ? "INGESTED" : "NO DATA"}</b></div></div></section>
