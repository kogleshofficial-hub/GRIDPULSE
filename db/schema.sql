...
DECLARE rid uuid; bucket text:=p_hash||':'||to_char(date_trunc('minute',now()),'YYYYMMDDHH24MI'); c integer;
BEGIN
...
INSERT INTO telemetry_reports(latitude,longitude,observed_at,status,source,region_name,external_id,reporter_hash) VALUES(p_lat,p_lon,p_observed,p_status,p_source,p_region,p_external,p_hash)
ON CONFLICT(source,external_id) DO UPDATE SET received_at=now() RETURNING id INTO rid;
...
END; $fn$;
CREATE OR REPLACE FUNCTION gridpulse.dashboard_summary() RETURNS TABLE(reports bigint,validated bigint,regions bigint,last_report timestamptz) LANGUAGE sql SECURITY DEFINER SET search_path=gridpulse,public AS $fn$
SELECT (SELECT count(*) FROM telemetry_reports),(SELECT count(*) FROM validation_metrics WHERE validation_status='validated'),(SELECT count(*) FROM region_clusters),(SELECT max(observed_at) FROM telemetry_reports); $fn$;
