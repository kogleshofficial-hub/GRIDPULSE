...
SELECT (SELECT count(*) FROM telemetry_reports),(SELECT count(*) FROM validation_metrics WHERE validation_status='validated'),(SELECT count(*) FROM validation_metrics WHERE validation_status='pending'),(SELECT count(*) FROM region_clusters WHERE active),(SELECT max(observed_at) FROM telemetry_reports),(SELECT count(DISTINCT region_name) FROM telemetry_reports WHERE observed_at>now()-interval '24 hours'); $fn$;
...
SELECT t.id,t.latitude,t.longitude,t.status,t.source,t.region_name,t.observed_at,COALESCE(v.confidence,0),COALESCE(v.validation_status,'pending') FROM telemetry_reports t LEFT JOIN validation_metrics v ON v.report_id=t.id ORDER BY t.observed_at DESC LIMIT LEAST(GREATEST(COALESCE(p_limit,100),1),250); $fn$;
...
SELECT r.id,r.name,r.center_lat,r.center_lon,count(t.id),count(t.id) FILTER (WHERE v.validation_status='validated'),max(t.observed_at) FROM region_clusters r LEFT JOIN telemetry_reports t ON t.region_id=r.id LEFT JOIN validation_metrics v ON v.report_id=t.id WHERE r.active GROUP BY r.id,r.name,r.center_lat,r.center_lon ORDER BY count(t.id) DESC,r.name; $fn$;
